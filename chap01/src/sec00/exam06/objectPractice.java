...
public class objectPractice {
	...
	public static void main(String[] args) {
		...
		// 메소드 선언은 선언부와 실행 블록으로 구성됩니다.(메소드 시그니처)
		...
		// 리턴값이 없는 메소드는 void로 선언합니다.
		// 다만 void로 선언된 메소드에서도 return문을 사용할 수 있는데, 
		// 리턴값을 지정하는 것이 아니라, 메소드 실행을 강제 종료시키는 역할을 합니다.
		
		// 메소드 오버로딩은 같은 이름의 메소드를 여러개 선언하는 것을 말합니다.
		// 하나의 메소드 이름으로 여러 기능을 담는다하여 붙여진 이름으로 매개변수의 타입, 개수, 순서 중 하나가 달라야 합니다.
		
		
		// 06-5 인스턴스 멤버와 정적 멤버
		// 정적은 '고정된'이란 의미입니다. 
		// 정적 멤버는 클래스에 고정된 멤버로써 객체를 생성하지 않고, 사용할 수 있는 필드와 메서드를 의미합니다.
		// 객체마다 가지고 있어야 할 데이터라면 인스턴스 필드로 선언하고,
		// 객체마다 가지고 있을 필요가 없는 공용 데이터라면 정적 필드로 선언하는 것이 좋습니다.
		// 정적 메서드인 static은 this 키워드를 사용할 수 없습니다.
		// 단, 하나의 객체를 생성하는 것을 싱글톤(Singleton)이라고 합니다.
		// final 필드는 초기값이 저장되면 실행 도중 수정할 수 없다는 것을 의미합니다. final 타입 필드 [=초기값];
		// 상수는 static final로 상수 이름은 모두 대문자로 작성하는 것이 관례입니다.
		
		// 패키지의 물리적인 형태는 파일 시스템의 폴더 입니다.
		// 패키지는 단순히 파일 시스템의 폴더 기능만 하는 것이 아니라,
		// 클래스의 일부분으로 클래스를 유일하게 만들어주는 식별자 역할을 합니다.
		// 패키지 명 중복을 방지하기 위해 회사의 도메인으로 패키지를 만듭니다.
		// 패키지 명은 역순으로 작성되어 집니다.
		
		// 접근제한자는 말 그대로 접근을 제한하기 위해 사용합니다.
		// 여기서 접근이란, 클래스 인터페이스 멤버의 접근을 의미합니다.
		// 제한 범위에 따라 public, protected, default, private
		
		// 클래스 상속
		// 현실에서 상속은 부모가 자식을 선택해서 물려주지만, 프로그램에서는 자식이 부모를 선택합니다.
		// 필드, 생성자, 메소드
		// 예를 들어, Car 클래스를 상속해서 SportsCar클래스를 설계하려면 다음과 같이 작성하면 됩니다.
	}
